...
public class CommonConfigPacket {
    private final LoversConfig.Common.SyncedValues values;

    public CommonConfigPacket(LoversConfig.Common.SyncedValues values) {
        this.values = values;
    }
    ...
    public static class Handler implements IModPacketHandler<CommonConfigPacket> {
        ...
        public CommonConfigPacket decode(PacketBuffer buf) {
            return new CommonConfigPacket(new LoversConfig.Common.SyncedValues(buf));
        }
